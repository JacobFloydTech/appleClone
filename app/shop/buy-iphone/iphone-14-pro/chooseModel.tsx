...
export default function ChooseModel(props: any) {
    ...
    return (
        <div className="w-auto ml-48 px-10 h-auto overflow-hidden relative">
            ...
            <div id="fade" className={`absolute border-black border-2 h-full bg-white bg-opacity-40 w-full top-[400px] z-30`}></div>
            ...
            <div>
                <div id='pro' onClick={() => { setCurrentlySelected("pro"); if (!selectedModel) { gsap.to("#fade", { top: `+=${200}px`, duration: 0.2 }); setSelectedModel(true); } }} className={"rounded-xl flex py-1 px-2 " + (currentlySelected == "pro" ? " border-blue-400 border-2" : " border-gray-400 border-2")}>
                ...
                <div id='max' onClick={() => { setCurrentlySelected("max"); if (!selectedModel) { gsap.to("#fade", { top: `+=${200}px`, duration: 0.2 }); setSelectedModel(true) } }} className={"rounded-xl flex py-1 px-2 mt-4 " + ((currentlySelected == "max" ? " border-blue-400 border-2" : " border-gray-400 border-2"))}>
                    <div className="my-auto block">
                ...
                <div className="mt-12">
                    ...
                    <div>
                        ...
                        <div className="flex mx-auto">
                            <div onMouseOver={() => { setColor({ ...color, current: "purple" }); }} onMouseLeave={() => { setColor({ ...color, current: "" }); }} onClick={() => { if (!color.clicked) { setColor({ ...color, clicked: true }); gsap.to("#fade", { top: "+=1000px" }) } }} className="rounded-full bg-purple-500 w-12 h-12"></div>
                            <div onMouseOver={() => { setColor({ ...color, current: "gray" }); }} onMouseLeave={() => { setColor({ ...color, current: "" }); }} onClick={() => { if (!color.clicked) { setColor({ ...color, clicked: true }); gsap.to("#fade", { top: "+=1000px" }) } }} className="rounded-full mx-4 bg-gray-500 w-12 h-12"></div>
                            <div onMouseOver={() => { setColor({ ...color, current: "gold" }); }} onMouseLeave={() => { setColor({ ...color, current: "" }); }} onClick={() => { if (!color.clicked) { setColor({ ...color, clicked: true }); gsap.to("#fade", { top: "+=1000px" }) } }} className="rounded-full  bg-yellow-500 w-12 h-12"></div>
                        </div>
                    </div>
                    <div className="mt-24">
                        <p className="text-3xl font-semibold">Storage. How much space do you need?</p>
                        <div onClick={() => { if (!storage.clicked) { gsap.to("#fade", { top: `+=${500}px`, duration: 0.2 }) }; setStorage({ storage: "128GB", clicked: true }) }} className={" border-2 rounded-xl flex mt-4 p-2" + (storage.storage == "128GB" ? " border-blue-500" : " border-gray-400")}>
                        ...
                        <div onClick={() => { if (!storage.clicked) { gsap.to("#fade", { top: `+=${500}px`, duration: 0.2 }) }; setStorage({ storage: "256GB", clicked: true }) }} className={" border-2 rounded-xl flex mt-4 p-2" + (storage.storage == "256GB" ? " border-blue-500" : " border-gray-400")}>
                        ...
                        <div onClick={() => { if (!storage.clicked) { gsap.to("#fade", { top: `+=${500}px`, duration: 0.2 }) }; setStorage({ storage: "512GB", clicked: true }) }} className={" border-2 rounded-xl flex mt-4 p-2" + (storage.storage == "512GB" ? " border-blue-500" : " border-gray-400")}>
                        ...
                        <div onClick={() => { if (!storage.clicked) { gsap.to("#fade", { top: `+=${500}px`, duration: 0.2 }) }; setStorage({ storage: "1TB", clicked: true }) }} className={" border-2 rounded-xl flex mt-4 p-2" + (storage.storage == "1TB" ? " border-blue-500" : " border-gray-400")}>
